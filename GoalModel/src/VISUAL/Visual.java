...
public class Visual extends JFrame {
	...
	private JCheckBoxMenuItem showEvaluation;
	...
	public Visual() {
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(Visual.class.getResource("/VISUAL/icons/vegan_white.png")));
		setTitle("VeGAn");
	...
	private JMenuBar generateMenu()
	{
		...
		importFromPiStarMenu.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				loadpiStarModel();
			}
		});
		...
		showEvaluation = new JCheckBoxMenuItem("Show evaluation");
		...
		settingsMenu.add(showEvaluation);
		...
		about.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
                ...
                frame.setIconImage(Toolkit.getDefaultToolkit().getImage(getClass().getResource("/VISUAL/icons/vegan_white.png")));
        		...
        		panel.add(new JLabel("<html>Copyright "+ Character.toString(169) + " 2022 Universitat Polit�cnica de Val�ncia,<br> Valencia, Spain</html>"));
	...
	private JPanel generateToolbar() {
		...
		importFrompiStarButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				loadpiStarModel();
			}
		});
...
private void loadpiStarModel()
{
	JFileChooser jfc = new JFileChooser();
	jfc.setCurrentDirectory(new File("."));
    jfc.setDialogTitle("Select a piStar Model");
    jfc.setAcceptAllFileFilterUsed(false);
    FileNameExtensionFilter filter = new FileNameExtensionFilter("piStar file", "json", "txt");
    jfc.addChoosableFileFilter(filter);
	
    int returnValue = jfc.showOpenDialog(this);
    if (returnValue == JFileChooser.APPROVE_OPTION) {
    	goalModelFile = jfc.getSelectedFile().getAbsolutePath();
    	
    	JsonFactory jsonFactory = new JsonFactory();
    	JsonParser jp = null;
		try {
			jp = jsonFactory.createJsonParser(new File(goalModelFile));
		} catch (IOException e) {
			
			JOptionPane.showMessageDialog(mainPanel, "Error loading the piStar Model.", "ERROR", JOptionPane.ERROR_MESSAGE);
			
			e.printStackTrace();
		}
		
		//The json not loads with the file://, but it is needed to SAVE the goal model
		goalModelFile = "file://" + goalModelFile;
    	...
    	try {
			...
			JOptionPane.showMessageDialog(mainPanel, "Error loading the json from the piStar Model.", "ERROR", JOptionPane.ERROR_MESSAGE);
			
			e.printStackTrace();
		}

    	goalModel = importPiStar.generateGoalModel(jsonNode);
...
private void prioritization()
{
	...
	for (Iterator<Actor> actorIterator = goalModel.getActors().iterator(); actorIterator.hasNext();) {
		...
		JComboBox<String> comboImportance = new JComboBox<String>(importances);
		...
		JComboBox<String> comboConfidence = new JComboBox<String>(confidences);
